...
public class LogNormalDistribution implements DiscreteDistribution{
    private static java.util.Random r = new java.util.Random();

    private int min = 0;
    private int max = 100;

    public LogNormalDistribution(int minM, int maxM){
        min = minM;
        max = maxM;
    }
    ...
    public void setCorr(double corr){
       // lambda = corr;
        System.out.println("Not applicatible, setCorr");
    }

    @Override
    public int generate(){
        double tempT = (logNormal() + 2.0)*0.25;
        while(tempT < 0.0 || tempT > 1.0){
            tempT = (logNormal() + 2.0)*0.25;
        }
        
        return min + (int) (tempT*(max-min));
    }

    @Override
    public double generateBasic(){
        double tempT = logNormal(); //(logNormal() + 3.0)*0.125;
        return tempT;
    }

    public double generateBasic(double tempI, double correlation){
         double tempT = generateBasic();
         double tempT1 = tempT * Math.sqrt(1.0 - Math.pow(correlation, 2)) + tempI * correlation;
         return tempT1;
    }

   public int generateFrom(double tempT){
        double tempI = tempT;
        return  (int)((max-min) * tempI + min);
    }
    
    public int generateFrom(double tempT, double tempI, double correlation){
        double tempT1 = tempI * Math.sqrt(1.0 - Math.pow(correlation, 2)) + tempT * correlation;
        return  (int)((max-min) * tempT1 + min);
     }
   ...
   public int getCount(){
    ...
    public double logNormal(){
        ...
        double sigma = Math.sqrt(Math.log(v/(m*m+1)));
        
        double y = generateNormal();
        double result = Math.exp(mu+y*sigma);
        return result;
    }
