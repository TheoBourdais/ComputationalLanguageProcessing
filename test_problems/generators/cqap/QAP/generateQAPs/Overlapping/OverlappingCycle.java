...
public class OverlappingCycle implements OverlappingStrategy{
    ...
    public OverlappingCycle(int nrF, int nrO, int nrM){
        nrFeatures = nrF;
    ...
    public OverlappingCycle(){}
    
    public int[][] compute(){
        int count = 0;
        for(int i = 0; i < nrMatrices-1; i++){
        ...
        int j = 0;
        sizeResult = sizeResultMatrix();
        while(count+j < sizeResult){
            overlapMatrix[nrMatrices-1][j] = count + j;
            j++;
        }
        if(j < nrFeatures){
            for(int i = 0; i < nrFeatureOverlap; i++){
                overlapMatrix[nrMatrices-1][i+j] = i;
            }
        }
    ...
    public int sizeResultMatrix(){
        sizeResult = nrMatrices * (nrFeatures - nrFeatureOverlap);
        return sizeResult;
    }
