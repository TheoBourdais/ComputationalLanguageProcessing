...
public class CorrelationDistanceFlows implements CorrelationGroups {   
    
    double[][] corrNum;
    CorrelationInterface corrInterf;
    
    public CorrelationDistanceFlows(double[][] corr, CorrelationInterface c){
        corrNum = corr;
        corrInterf = c;
    }
        ...
        for(int i = 0; i < output.length; i++){
            if(corrInterf.typeCorrelation() == 0){
                if(correlation[0][1] > 0){
                    output[i] = corrInterf.correlation(inputs[0], correlation[0][i], distr);
                } else {
                    output[i] = 1.0 - corrInterf.correlation(inputs[0], -correlation[0][i], distr);
                }
            } else {
                output[i] = corrInterf.correlation(inputs[0], correlation[0][i], distr);
            }
