...
public class CopyFillUpMatrix extends FillUpMatrix{
    ...
    public CopyFillUpMatrix(DiscreteDistribution fillD, DiscreteDistribution fillF){
        fillFlow = fillF;
        fillDistance = fillD;
    }

    @Override public void setNrFacilities(int nrF){
        nrFacilities = nrF;
    }

    @Override public void setNrObjectives(int nrO){
    }

    @Override public void setNrMatrices(int nrM){
        nrMatrices = nrM;
    }
            ...
            for(int j = 0; j < nrFacilities; j++){
               ...
               for(int m = 0; m < nrMatrices; m++){
                if(i < tableOver[1][m] && i >= tableOver[0][m] && j < tableOver[1][m] && j >= tableOver[0][m]){
                    ...
                    indexF[0] = i;
                    indexF[1] = j;
                    newFlow.setElement(indexF, newFlow.getElement(index) +((GenerateQAPs)listP[m]).getFlow().getFlow()[i- tableOver[0][m]][j- tableOver[0][m]]);
                    nonZero = true;
                }
              }
